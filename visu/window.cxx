...
namespace Window {
  ...
  void init(int width, int height, std::string titre, int argc, char** argv, std::string keyboard, int p_fps, float translation_speed) {
    ...
    camera = new Camera(mode, -100, 100, -100, 4*M_PI/7, M_PI/4, 0.01, translation_speed, WIDTH, HEIGHT1);
    fps_goal = p_fps;
  }
  ...
  void launch() {
    ...
    glutWarpPointer(WIDTH/2, HEIGHT1/2);
    glutMainLoop();
  }

  void mouseMove(int x, int y) {
    camera->rotation(x, y);
    if(x>=WIDTH || x<=0 || y>=HEIGHT1 || y<=0) {
      glutWarpPointer(WIDTH/2, HEIGHT1/2);
      camera->setMouse(WIDTH/2, HEIGHT1/2);
    }
  }
